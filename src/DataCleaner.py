...
data_directory = os.path.join(parent_dir, "data/processed_data")
...
def is_valid(data: Dict) -> bool:
    ...
    """
    is_how_much = (
        False if re.search("how much", data["question"], re.IGNORECASE) else True
    )
...
def remove_long_entries(entries: List[Dict]) -> List[Dict]:
    ...
    return [
        ...
        if sum(len(message["content"]) for message in entry["messages"]) <= 2048
    ]
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename=f"{parent_dir}/log/DataCleaner.log",
        encoding="utf-8",
        level=logging.DEBUG,
        filemode="w",
    )
